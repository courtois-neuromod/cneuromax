...
from mmdit.embed.clip_and_t5_text_emb import MixedTextEmbedder
from mmdit.embed.patch_emb import PatchEmbed3D
from mmdit.embed.pos_emb import (
    get_1d_sincos_pos_embed,
    get_2d_sincos_pos_embed,
)
from mmdit.embed.time_emb import TimestepEmbedder
from mmdit.modules.mmdit_block import MMDiTBlock, MMDiTFinalLayer
from mmdit.utils.operation import gather_forward_split_backward
...
class MMDiT(nn.Module):
    ...
    def __init__(
        ...
        if self.use_video:
            ...
            self.c_embedder = MixedTextEmbedder(
                clip_text_encoder, t5_text_encoder, hidden_size
            )
            self.num_patches = np.prod(
                [input_size[i] // patch_size[i] for i in range(3)]
            )
        else:
        ...
        if self.use_video:
            ...
            self.register_buffer(
                "pos_embed_spatial", self.get_spatial_pos_embed()
            )
            self.register_buffer(
        ...
        self.blocks = nn.ModuleList(
            [
                MMDiTBlock(
                    ...
                    hidden_size,
                    num_heads,
                    enable_layernorm_kernel or enable_modulate_kernel,
                    mlp_ratio,
        ...
        self.final_layer = MMDiTFinalLayer(
            hidden_size,
            np.prod(patch_size) if use_video else patch_size ** 2,
            self.out_channels,
        )
    ...
    def forward(self, x, t, c):
        ...
        text_embedding_spatial = repeat(
            c, "n d -> (n c) d", c=self.temp_embed.shape[1]
        )
        text_embedding_temp = repeat(
            c, "n d -> (n c) d", c=self.pos_embed.shape[1]
        )
        ...
        if self.sequence_parallel_size > 1:
            x = x.chunk(self.sequence_parallel_size, dim=1)[
        ...
        for block in self.blocks:
            if self.gradient_checkpointing:
                x = torch.utils.checkpoint.checkpoint(
                    self.create_custom_forward(block), t, x, c
                )
            else:
                x = block(t, x, c)  # (N, T, D)
        ...
        if self.sequence_parallel_size > 1:
            x = gather_forward_split_backward(
                x, dim=1, process_group=self.sequence_parallel_group
            )
