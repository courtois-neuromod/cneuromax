...
from mmdit.embed.clip_text_emb import TextEmbedder
from mmdit.embed.label_emb import LabelEmbedder
...
from mmdit.modules.block import DiTBlock, FinalLayer
    ...
    def __init__(
        ...
        if self.use_video:
            ...
            self.y_embedder = TextEmbedder(
                path=text_encoder, hidden_size=hidden_size
            )
        ...
        for block in self.blocks:
            if self.gradient_checkpointing:
                x = torch.utils.checkpoint.checkpoint(
                    self.create_custom_forward(block), x, c
                )
            else:
                x = block(x, c)  # (N, T, D)
