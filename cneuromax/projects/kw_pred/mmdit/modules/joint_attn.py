...
"""
from mmdit.utils.operation import (
    AllGather,
...
class JointAttention(nn.Module):
    ...
    def forward(self, x, c):
        ...
        num_heads = (
            ...
            else self.num_heads // self.sequence_parallel_size
        )
        """
        ### NEW ###
        ...
        q_text, k_text, v_text = self.split_qkv(
            qkv_text, B, N, total_N, num_heads
        )
        """
        ...
        """
            ...
            z = ring_flash_attn_cuda(
                q, k, v, causal=self.causal, bucket_size=self.ring_bucket_size
            )
        ...
        if (
            ...
            or self.attention_mode == "ring"
            or self.attention_mode == "rebased"
        ):
            # z = z.reshape(x_output_shape)
            z = z.reshape(z_output_shape)
        else:
            # z = z.transpose(1, 2).reshape(x_output_shape)
            z = z.transpose(1, 2).reshape(z_output_shape)

        """
        if self.sequence_parallel_size > 1:
            z = all_to_all_comm(
                z, self.sequence_parallel_group, scatter_dim=1, gather_dim=2
            )
        """
    ...
    def rearrange_fused_weight(self, layer: nn.Linear, flag="load"):
        ...
        if not isinstance(layer, nn.Linear):
            raise ValueError(
                "Invalid layer type for fused qkv weight rearrange!"
            )

        with torch.no_grad():
            if flag == "load":
                layer.weight.data = rearrange(
                    layer.weight.data,
                    "(x NH H) D -> (NH x H) D",
                    x=3,
                    H=self.head_dim,
                )
                layer.bias.data = rearrange(
                    layer.bias.data,
                    "(x NH H) -> (NH x H)",
                    x=3,
                    H=self.head_dim,
                ...
                raise ValueError(
                    "Invalid flag for fused qkv weight rearrange!"
                )
    """
