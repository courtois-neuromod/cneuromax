...
from mmdit.modules.joint_attn import JointAttention
...
def modulate(norm_func, x, shift, scale, use_kernel=False):
    ...
    if use_kernel:
        try:
            from mmdit.kernels.fused_modulate import fused_modulate

            x = fused_modulate(x, scale, shift)
        except ImportError:
            raise RuntimeError(
                "FusedModulate kernel not available. Please install triton."
            )
    else:
...
class MMDiTBlock(nn.Module):
    ...
    def __init__(
        ...
        **block_kwargs,
        ...
        self.text_y_input = nn.Sequential(
            nn.SiLU(), nn.Linear(y_size, self.num_params, bias=False)
        )
        ...
        self.text_norm1 = get_layernorm(
            text_hidden_size, 1e-6, False, self.use_kernel
        )
        self.pix_norm1 = get_layernorm(
            pix_hidden_size, 1e-6, False, self.use_kernel
        )
        ...
        self.text_norm2 = get_layernorm(
            text_hidden_size, 1e-6, False, use_kernel
        )
        self.pix_norm2 = get_layernorm(
            pix_hidden_size, 1e-6, False, use_kernel
        )
        ...
        approx_gelu = (
            lambda: nn.Identity()
        )  # act funcs are in the other places in SD3 to improve speed
        self.text_mlp = Mlp(
            in_features=text_hidden_size,
            hidden_features=text_mlp_hidden_dim,
            act_layer=approx_gelu,
            drop=0,
        )
        self.pix_mlp = Mlp(
            in_features=pix_hidden_size,
            hidden_features=pix_mlp_hidden_dim,
            act_layer=approx_gelu,
            drop=0,
        )
    ...
    def forward(self, y, x, c):
        ...
        def bc(tnsr):
            return tnsr.view(
                -1, 1, 1, 1, 1
            )  # broadcasting for batched scalar-tensor multiplication
        ...
        alpha_txt, beta_txt, gamma_txt, delta_txt, eps_txt, dz_txt = (
            coefs_text.unbind(-1)
        )
        alpha_pix, beta_pix, gamma_pix, delta_pix, eps_pix, dz_pix = (
            coefs_pix.unbind(-1)
        )
        ...
        x, c = self.attn(x, c)
        x_new = self.pix_norm2(x * bc(gamma_pix) + x) * bc(
            delta_pix
        ) + eps_pix * torch.ones_like(x)
        c_new = self.text_norm2(c * bc(gamma_txt) + c) * bc(
            delta_txt
        ) + eps_txt * torch.ones_like(c)
...
class MMDiTFinalLayer(nn.Module):
    ...
    def forward(self, x, c):
        ...
        x = modulate(
            nn.Identity(), x, shift, scale, use_kernel=self.use_kernel
        )
        x = self.linear(x)
