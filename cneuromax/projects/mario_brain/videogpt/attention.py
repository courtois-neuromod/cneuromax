# Adapted from https://github.com/wilson1yan/VideoGPT/blob/master/videogpt/attention.py
import numpy as np
import torch
...
from .utils import tensor_slice, view_range
    ...
    def forward(self, x, cond, decode_step, decode_idx):
        ...
        if self.use_frame_cond:
            ...
            if self.training:
                ...
                h = self.enc_attn(
                    h, cond_data, cond_data, decode_step, decode_idx
                )
...
class MultiHeadAttention(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.causal = causal
        self.shape = shape  # downsampled (t, h, w)
...
class AxialAttention(nn.Module):
    ...
    def forward(self, q, k, v, decode_step, decode_idx):
        q = q.movedim(self.axial_dim, -2).flatten(end_dim=-3)
        k = k.movedim(self.axial_dim, -2).flatten(end_dim=-3)
        v = v.movedim(self.axial_dim, -2)
        old_shape = list(v.shape)
        v = v.flatten(end_dim=-3)

        out = scaled_dot_product_attention(q, k, v, training=self.training)
        out = out.view(*old_shape)
        out = out.movedim(-2, self.axial_dim)
        return out
...
class SparseAttention(nn.Module):
    ...
    def get_ops(self):
        try:
            ...
            raise Exception(
                "Error importing deepspeed. Please install using "
    ...
    def forward(self, q, k, v, decode_step, decode_idx):
        ...
        SparseAttention.block_layout[self.shape] = (
            SparseAttention.block_layout[self.shape].to(q)
        )
        if self.causal:
            SparseAttention.attn_mask[self.shape] = (
        ...
        attn_mask = (
            SparseAttention.attn_mask[self.shape] if self.causal else None
        )
...
class RightShift(nn.Module):
    ...
    def forward(self, x, decode_step):
        ...
        )  # (b, t_d * h_d * w_d, embd_dim)
        sos = (
            torch.ones(x_shape[0], 1, self.embd_dim, dtype=torch.float32).to(
                self.sos
            )
            * self.sos
        )
...
class LayerNorm(nn.Module):
    def __init__(self, embd_dim, class_cond_dim):
        ...
        if self.conditional:
            ...
            nn.init.constant_(
                self.w.weight.data, 1.0 / np.sqrt(class_cond_dim)
            )
